...
download_xray_release_zip() {
  ...
  if ! curl -fL --retry 3 --retry-delay 2 --connect-timeout 15 --max-time 300 -H "User-Agent: Sahar/0.1.70" "$download_url" -o "$output_zip"; then
    set_fail_hint "Failed to download Xray archive"
    return 1
  fi
  digest_file="$(mktemp)"
  if curl -fL --retry 2 --retry-delay 2 --connect-timeout 15 --max-time 60 -H "User-Agent: Sahar/0.1.70" "$digest_url" -o "$digest_file"; then
