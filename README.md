...
![version](https://img.shields.io/badge/version-0.1.22-8b5cf6)
...
```bash
unzip sahar_0.1.22_single_installer.zip && cd sahar_0.1.22_single_installer && sudo sh install.sh
```

برای نصب مستقیم Master:

```bash
unzip sahar_0.1.22_single_installer.zip && cd sahar_0.1.22_single_installer && sudo sh install.sh master
```

برای نصب مستقیم Agent:

```bash
unzip sahar_0.1.22_single_installer.zip && cd sahar_0.1.22_single_installer && sudo sh install.sh agent
```
...
## رفع خطای نصب Xray روی Alpine / Fix Xray install issue on Alpine

اگر هنگام نصب روی Alpine با خطاهایی مثل `curl: (22) ... 403` یا `JSONDecodeError` در مرحله‌ی نصب Xray روبه‌رو شدید، این مشکل معمولاً به محدود شدن دسترسی سرور به GitHub API برمی‌گردد. از نسخه‌ی `0.1.22` به بعد، installer دیگر برای Alpine به GitHub API وابسته نیست و ابتدا از لینک مستقیم `latest/download` استفاده می‌کند و فقط در صورت نیاز fallback می‌زند.

If you see errors such as `curl: (22) ... 403` or `JSONDecodeError` while installing Xray on Alpine, the server is usually being blocked or rate-limited by the GitHub API. Starting from version `0.1.22`, the installer no longer depends on the GitHub API for Alpine and uses the direct `latest/download` asset URL first, with a safer fallback path.
