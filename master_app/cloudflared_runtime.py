...
def _release_asset_metadata(asset_name: str) -> tuple[str, str]:
    ...
    req = urllib.request.Request(url, headers={'Accept': 'application/vnd.github+json', 'User-Agent': 'Sahar/0.1.72'})
...
def install_binary(dest_path: str = BINARY_PATH) -> str:
    ...
    os.close(fd)
    try:
        req = urllib.request.Request(url, headers={'User-Agent': 'Sahar/0.1.72'})
