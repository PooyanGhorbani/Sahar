...
from panel_logic import build_dashboard_text, merge_server_runtime_update
from utils import (
    add_days,
    build_vless_link,
    calc_expire,
...
def format_user_brief(user: Dict[str, Any]) -> str:
    ...
    server_name = _safe_text(user.get('server_name'))
    used = user.get('used_gb', 0)
    traffic = user.get('traffic_gb', 0)
    expire_date = _safe_text(user.get('expire_date'))
    return f"{status} <b>{username}</b> | {server_name} | {used}/{traffic} GB | {expire_date}"
...
def status_text() -> str:
    ...
    expired_count = len(DB.list_expired_active_users(today_utc()))
    quota_count = len(DB.list_over_quota_active_users())
    error_count = len(DB.list_error_events(limit=20))
    return build_dashboard_text(
        users=users,
        servers=servers,
        version=config.get('package_version') or '-',
        bot_state=bot_state,
        scheduler_state=scheduler_state,
        subscription_state=subscription_state,
        local_agent_state=local_agent_state,
...
def refresh_server_metadata(server_name: str) -> Tuple[Dict[str, Any], Dict[str, Any]]:
    ...
    DB.add_or_update_server(merge_server_runtime_update(server, health, now_iso()))
...
def _is_local_server(server: Dict[str, Any]) -> bool:
    ...
    return (
        ...
        or api_url.startswith('https://127.0.0.1:')
        or api_url.startswith('https://localhost:')
    )
...
def main_menu_markup() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        ...
        [_menu_button('🔎 جستجو', 'act:prompt:find_any'), _menu_button('📊 گزارش‌ها', 'menu:reports')],
        [_menu_button('🧰 ابزارها', 'menu:tools'), _menu_button('➕ ساخت کاربر', 'wizard:create_user')],
        [_menu_button('🚀 افزودن سرور با SSH', 'wizard:add_server_ssh'), _menu_button('❓ راهنما', 'menu:help')],
    ])
...
def tools_menu_markup() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        ...
        [_menu_button('🧯 آخرین خطاها', 'tool:last_errors'), _menu_button('🫀 گزارش سلامت', 'tool:health_report')],
...
def xray_status_text(server_name: str) -> str:
    ...
    try:
        ...
        return (
            f"<b>Xray Status — {_safe_text(server_name)}</b>\n"
            f"🔘 Server enabled: {'بله' if server.get('enabled') else 'خیر'}\n"
            f"💚 Health: {_safe_text(server.get('last_health_status') or 'unknown')}\n"
            f"⚙️ Xray active: {'بله' if health.get('xray_active') else 'خیر'}\n"
            f"📡 Public host: {_safe_text(cfg.get('public_host') or server.get('public_host') or '-')}\n"
            f"🔌 Simple port: {_safe_text(cfg.get('simple_port') or server.get('xray_port') or '-')}\n"
            f"🔐 Reality port: {_safe_text(cfg.get('reality_port') or server.get('reality_port') or '-')}\n"
            f"🚚 Transport mode: {_safe_text(cfg.get('transport_mode') or server.get('transport_mode') or '-')}\n"
...
def server_profiles_text(server_name: str) -> str:
    ...
    for profile in profiles:
        lines.append(
            f"• {_safe_text(profile.get('display_name') or profile.get('profile_key'))} | host={_safe_text(profile.get('public_host') or '-')} | port={_safe_text(profile.get('port') or '-')} | transport={_safe_text(profile.get('transport_mode') or '-')} | path={_safe_text(profile.get('ws_path') or '-')}"
        )
...
def health_report_text() -> str:
    ...
    for server in servers:
        ...
        lines.append(f"{icon} <b>{_safe_text(server.get('name'))}</b> | state={_safe_text(server.get('provisioning_state') or '-')} | health={_safe_text(server.get('last_health_status') or '-')} | xray={'on' if server.get('xray_active') else 'off'}")
...
def doctor_text() -> str:
    ...
    return '\n'.join([
        ...
        f'• Down servers: <code>{html.escape(", ".join(down_servers)) if down_servers else "-"}</code>',
...
def user_text(user: Dict[str, Any]) -> str:
    ...
    return (
        f"<b>کاربر: {_safe_text(user.get('username'))}</b>\n"
        f"🖥 سرور: {_safe_text(user.get('server_name'))}\n"
        f"🆔 UUID: <code>{_safe_text(user.get('uuid'))}</code>\n"
        f"📦 حجم: {user.get('traffic_gb', 0)} GB\n"
        f"📊 مصرف: {user.get('used_gb', 0)} GB\n"
        f"📅 انقضا: {_safe_text(user.get('expire_date'))}\n"
        f"💳 کریدیت: {user.get('credit_balance', 0)}\n"
        f"🏷 پلن: {html.escape(resolve_plan_label(user.get('plan') or ''))}\n"
        f"📌 یادداشت: {_safe_text(user.get('notes'))}\n"
...
def server_text(server: Dict[str, Any]) -> str:
    return (
        f"<b>سرور: {_safe_text(server.get('name'))}</b>\n"
        f"🌐 API: <code>{_safe_text(server.get('api_url'))}</code>\n"
        f"📡 Public host: {_safe_text(server.get('public_host'))}\n"
        f"🔌 Port: {_safe_text(server.get('xray_port'))}\n"
        f"🚚 Transport: {_safe_text(server.get('transport_mode') or 'ws')}\n"
        f"🛣 WS Path: {_safe_text(server.get('ws_path') or '/ws')}\n"
        f"💚 Health: {_safe_text(server.get('last_health_status') or 'unknown')}\n"
        f"📝 Health msg: {_safe_text(server.get('last_health_message'))}\n"
        f"⏰ Last check: {_safe_text(server.get('last_health_at'))}\n"
        f"🧠 CPU: {server.get('cpu_percent', 0)}% | RAM: {server.get('memory_percent', 0)}% | Disk: {server.get('disk_percent', 0)}%\n"
        f"📈 Load: {server.get('load_1m', 0)} | 👥 Users: {server.get('user_count', 0)} | 🔄 Last sync: {_safe_text(server.get('last_sync_at'))}\n"
        f"☁️ DNS: {_safe_text(server.get('cf_dns_name'))}\n"
        f"🚇 Tunnel: {_safe_text(server.get('cf_tunnel_name'))} | {_safe_text(server.get('cf_tunnel_status'))}\n"
        f"🧭 مرحله: {_safe_text(server.get('provisioning_state'))}\n"
        f"💬 پیام مرحله: {_safe_text(server.get('provisioning_message'))}\n"
        f"🔘 وضعیت: {'فعال' if server.get('enabled') else 'غیرفعال'}"
    )
...
def search_results_markup(users: List[Dict[str, Any]], servers: List[Dict[str, Any]], *, include_servers: bool = True) -> InlineKeyboardMarkup:
    rows: List[List[InlineKeyboardButton]] = []
    for user in users[:8]:
        rows.append([_menu_button(f"👤 {user['username']}", f"user:{user['username']}")])
    for server in servers[:6]:
        rows.append([_menu_button(f"🌐 {server['name']}", f"server:{server['id']}")])
    if include_servers:
        rows.append([_menu_button('🔁 جستجوی دوباره', 'act:prompt:find_any')])
    else:
        rows.append([_menu_button('🔁 جستجوی کاربر', 'act:prompt:search_users')])
    rows.append([_menu_button('🏠 خانه', 'menu:home')])
    return InlineKeyboardMarkup(rows)


def build_combined_search_result(
    query: str,
    ...
    *,
    ...
    shown_users = users[:8]
    shown_servers = servers[:6]
    hidden_users = max(len(users) - len(shown_users), 0)
    hidden_servers = max(len(servers) - len(shown_servers), 0)
    total_hits = len(users) + (len(servers) if include_servers else 0)
    lines: List[str] = [f'🔎 عبارت: <code>{html.escape(query)}</code>', f'📦 نتیجه‌ها: {total_hits}']
    if users:
        lines.append(f'👥 کاربران: {len(users)}')
        lines.extend(format_user_brief(u) for u in shown_users)
        if hidden_users:
            lines.append(f'… {hidden_users} کاربر دیگر هم پیدا شد. برای جزئیات بیشتر جستجو را محدودتر کن.')
    else:
        lines.append('👥 کاربران: 0')
    if include_servers:
        lines.append('')
        if servers:
            lines.append(f'🌐 سرورها: {len(servers)}')
            for server in shown_servers:
                health = _safe_text(server.get('last_health_status') or 'unknown')
                target = _safe_text(server.get('cf_dns_name') or server.get('public_host') or '-')
                lines.append(f"• <b>{_safe_text(server.get('name'))}</b> — {health} — {target}")
            if hidden_servers:
                lines.append(f'… {hidden_servers} سرور دیگر هم پیدا شد. برای جزئیات بیشتر جستجو را محدودتر کن.')
        else:
            lines.append('🌐 سرورها: 0')
    if total_hits == 0:
        lines.extend([
            '',
            'موردی پیدا نشد.',
            'نام کاربر، نام سرور، دامنه، پلن یا بخشی از یادداشت را امتحان کن.',
        ])
    return list_text('نتیجه جستجو', lines), search_results_markup(users, servers if include_servers else [], include_servers=include_servers)
...
async def show_users(update: Update, context: ContextTypes.DEFAULT_TYPE, page: int = 0) -> None:
    ...
    lines.insert(0, f'تعداد کل: {len(users)} | فعال: {sum(1 for u in users if u.get("is_active"))}')
...
async def show_servers(update: Update, context: ContextTypes.DEFAULT_TYPE, page: int = 0) -> None:
    ...
    lines = [f'تعداد کل: {len(servers)} | سالم: {sum(1 for s in servers if s.get("enabled") and (s.get("last_health_status") or "") == "ok")}']
    for s in page_servers:
        status = s.get('last_health_status') or 'unknown'
        target = s.get('cf_dns_name') or s.get('public_host') or '-'
        lines.append(f"• <b>{_safe_text(s.get('name'))}</b> — {_safe_text(status)} — {'enabled' if s['enabled'] else 'disabled'} — {_safe_text(target)}")
...
async def show_help(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    text = (
        ...
        '• از دکمه «🔎 جستجو» یا دستور <code>/find</code> برای جستجوی کاربر و سرور استفاده کن.\n'
        '• سطح دسترسی‌ها: مالک، ادمین، پشتیبان.\n\n'
        'دستورهای متنی اصلی:\n'
        '/start /panel /status /help /find <query>\n'
...
async def search_users_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    rendered_text, markup = build_combined_search_result(query, users, [], include_servers=False)
    await respond(update, rendered_text, markup)


@admin_only
async def find_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not context.args:
        set_prompt(context, 'find_any')
        await send_temp(update, 'عبارت جستجو را بفرست. کاربر و سرور هم‌زمان بررسی می‌شوند.')
        return
    query = ' '.join(context.args).strip()
    users = DB.search_users(query)
    servers = DB.search_servers(query)
    rendered_text, markup = build_combined_search_result(query, users, servers)
    await respond(update, rendered_text, markup)
...
async def callback_router(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        if data == 'menu:home':
            ...
            await show_plans(update, context)
        elif data == 'tool:last_errors':
            await last_errors_cmd(update, context)
        elif data == 'tool:health_report':
            await health_report_cmd(update, context)
            ...
            prompt_map = {
                ...
                'find_any': 'عبارت جستجو را بفرست. کاربر و سرور هر دو بررسی می‌شوند.',
...
async def prompt_text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        if action == 'search_users':
            ...
            rendered_text, markup = build_combined_search_result(text, users, [], include_servers=False)
            await respond(update, rendered_text, markup)
            return
        ...
        if action == 'find_any':
            clear_prompt(context)
            users = DB.search_users(text)
            servers = DB.search_servers(text)
            rendered_text, markup = build_combined_search_result(text, users, servers)
            await respond(update, rendered_text, markup)
            return
...
def main() -> None:
    ...
    application.add_handler(CommandHandler('search', find_cmd))
