...
def load_config(path: str) -> Dict:
    ...
    if 'cloudflare_tunnel_enabled' not in data:
        if 'cloudflare_argo_enabled' in data:
            data['cloudflare_tunnel_enabled'] = _as_bool(data.get('cloudflare_argo_enabled'))
        elif 'cloudflare_argo_tunnel_enabled' in data:
            data['cloudflare_tunnel_enabled'] = _as_bool(data.get('cloudflare_argo_tunnel_enabled'))
    else:
        data['cloudflare_tunnel_enabled'] = _as_bool(data.get('cloudflare_tunnel_enabled'))
    if 'cloudflare_argo_enabled' not in data:
        data['cloudflare_argo_enabled'] = _as_bool(data.get('cloudflare_tunnel_enabled', False))
    else:
        data['cloudflare_argo_enabled'] = _as_bool(data.get('cloudflare_argo_enabled'))
...
def save_config(path: str, data: Dict) -> None:
    ...
    payload['cloudflare_tunnel_enabled'] = _as_bool(payload.get('cloudflare_tunnel_enabled', payload.get('cloudflare_argo_enabled', False)))
    payload['cloudflare_argo_enabled'] = _as_bool(payload.get('cloudflare_tunnel_enabled', False))
...
def setup_logging(log_path: str) -> None:
    ...
    configured_paths = getattr(root_logger, '_sahar_logging_paths', set())
