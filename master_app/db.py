...
class Database:
    ...
    def search_users(self, pattern: str) -> List[Dict[str, Any]]:
        ...
        with self.connect() as conn:
            rows = conn.execute(
                ...
                SELECT u.*, s.name AS server_name
                FROM users u JOIN servers s ON s.id = u.server_id
                WHERE u.username LIKE ? OR u.plan LIKE ? OR u.notes LIKE ? OR s.name LIKE ?
                ORDER BY u.username ASC
                LIMIT 100
                ''',
                (like, like, like, like),
    ...
    def search_servers(self, pattern: str) -> List[Dict[str, Any]]:
        like = f'%{pattern}%'
        with self.connect() as conn:
            rows = conn.execute(
                '''
                ...
                WHERE name LIKE ?
                   OR public_host LIKE ?
                   OR cf_dns_name LIKE ?
                   OR provisioning_message LIKE ?
                   OR last_health_message LIKE ?
                ORDER BY name ASC
                LIMIT 100
                ''',
